...
// Helper: delete media file (local or Cloudinary)
...
export const createPost = async (req, res) => {
  const { caption, mediaUrl: bodyMediaUrl } = req.body;
  let mediaUrl  = "";
  let mediaType = "text";
  ...
  if (req.file) {
    ...
    // Only delete local file if successfully uploaded to Cloudinary
    if (result.isCloud) {
...
export const getFeed = async (req, res) => {
  ...
  const [followingPosts, otherPosts] = await Promise.all([
    Post.find({ author: { $in: followingIds } }).sort({ createdAt: -1 }).populate("author", "id username name avatar"),
    Post.find({ author: { $nin: [...followingIds, req.user._id] } }).sort({ createdAt: -1 }).populate("author", "id username name avatar"),
  ]);
...
export const getReels = async (req, res) => {
  ...
  const allVideos = await Post.find({ mediaType: "video" }).populate("author", "id username name avatar");
  shuffle(allVideos);
...
export const getPost = async (req, res) => {
  const Comment = (await import("../models/Comment.js")).default;
  if (!req.params.id || req.params.id === "undefined") return res.status(400).json({ message: "Invalid post ID" });
...
export const getUserPosts = async (req, res) => {
  ...
  const [posts, total] = await Promise.all([
    Post.find({ author: req.params.userId }).sort({ createdAt: -1 }).skip((page - 1) * limit).limit(limit).populate("author", "id username name avatar"),
...
// PUT /api/posts/:id — edit caption (and optionally replace media)
export const editPost = async (req, res) => {
  ...
  // New media uploaded — delete old, upload new
  if (req.file) {
  ...
  const updated = await Post.findByIdAndUpdate(post._id, data, { new: true, returnDocument: "after" })
...
export const sharePost = async (req, res) => {
  const post = await Post.findByIdAndUpdate(req.params.id, { $inc: { shares: 1 } }, { new: true, returnDocument: "after" });
